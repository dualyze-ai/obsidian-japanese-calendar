...
> - **カレンダーの行の高さ**: 吉凶日を非表示にしている場合でも各行の高さは吉凶日表示を前提に固定されているため、上下に余白が目立つことがあります。気になる場合は `styles.css` の `.jhc-days-grid` にある `grid-auto-rows`（デフォルト `52px`）の値を小さく（例: `36px`）調整してください。
